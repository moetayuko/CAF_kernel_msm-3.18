 ...
 * use these defines to set cyttsp_platform_data.gen in board configuration file
 */
...
#define CYTTSP_USE_GEST_GRP1		/* Group 1 is the default */
...
/* define only if not defined already by system; value based on linux kernel 2.6.30.10 */
	...
	#define cyttsp_testdat(ray1, ray2, sizeofray) \
		{int i; u8 *up1=(u8 *)ray1, *up2=(u8 *)ray2; \
		for (i = 0; i < sizeofray; i++){up1[i] = up2[i];}}
#else
...
#define FLIP_XY(x,y)			{u16 tmp; tmp = x; x = y; y = tmp;}
...
#define CYTTSP_NUM_RETRY		4	/* per TRM; max num retries to read touch data */
...
/* Bootloader keys */
#define CYTTSP_BL_KEYS_DFLT		0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07
#define CYTTSP_BL_KEYS			CYTTSP_BL_KEYS_DFLT	/* edit this field for custom keys */
...
#define CYTTSP_SOFT_RESET_MODE		0x01	/* to return to Bootloader mode */
...
#define CYTTSP_GEN2_2TOUCH			0x03	/* two touch; no ghost */
#define CYTTSP_GEN2_1TOUCH			0x01	/* one touch only */
#define CYTTSP_GEN2_TOUCH2			0x01	/* First touch removed; sencond touch remains */
...
#define cyttsp_wake_data_t	cyttsp_gen3_xydata_t
		...
		/* this file declares firmware download block array (cyttsp_fw[])
		 * and the number of command block records
		 * (cyttsp_fw_records)
		 */
		#ifdef CYTTSP_USE_I2C
			#include "cyttsp_fw-i2c.h"		/* imports cyttsp_fw[] array */
		#else
			#include "cyttsp_fw-spi.h"
		#endif
		#define cyttsp_app_load()	1
		#ifdef CYTTSP_FW_UPDATE
			#define cyttsp_force_fw_load()	1
		#else
			#define cyttsp_force_fw_load()	0
		#endif
		
		/* the following defines reference the variables declared in the load file */
	#else
		/* the following array declaration is to allow some debugging capability */
