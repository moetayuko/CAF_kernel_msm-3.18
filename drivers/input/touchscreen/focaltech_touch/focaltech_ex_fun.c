			...
			size_t count, loff_t *ppos)
{
	...
	case PROC_UPGRADE:
	{
		...
		snprintf(upgrade_file_path, sizeof(upgrade_file_path),
			"%s", writebuf + 1);
	...
	case PROC_SET_SLAVE_ADDR:
		...
		if (writebuf[1] != fts_i2c_client->addr) {
			...
			FTS_DEBUG("Change i2c addr 0x%x to 0x%x",
				ret<<1, writebuf[1]<<1);
	...
	case PROC_WRITE_DATA:
		writelen = count - 1;
		if (writelen > 0) {
			ret = fts_i2c_write(fts_i2c_client,
				writebuf + 1, writelen);
			if (ret < 0)
				FTS_ERROR("[APK]: write iic error!!");
		}
		...
		size_t count, loff_t *ppos)
{
	...
	case PROC_UPGRADE:
		...
		if (ret < 0)
			...
			num_read_chars = snprintf(buf, sizeof(buf), "%s",
				"get fw version failed.\n");
		else
			/*num_read_chars = sprintf(buf,
				"current fw version:0x%02x\n", regvalue);*/
			num_read_chars = snprintf(buf, sizeof(buf),
				"current fw version:0x%02x\n", regvalue);
		break;
	case PROC_READ_REGISTER:
		readlen = 1;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (0 != retval) {
		/* Could not convert the given input to a number.
		*  The given input was:
		*/
		FTS_ERROR("%s()-ERROR:The given input was: \"%s\"",
				__func__, buf);
		goto error_return;
	}
#if FTS_ESDCHECK_EN
	...
	if (2 == num_read_chars) {
		...
		/*read register*/
		regaddr = wmreg;
		g_rwreg_result.result = fts_i2c_read_reg(client,
					regaddr, &regvalue);
		if (g_rwreg_result.result < 0) {
			FTS_ERROR("Could not read the register(0x%02x)",
					regaddr);
		} else {
			FTS_INFO("the register(0x%02x) is 0x%02x",
				regaddr, regvalue);
			...
			struct device_attribute *attr, char *buf)
{
	...
	count = snprintf(buf, sizeof(buf), "Esd check: %s\n",
		...
		struct device_attribute *attr, char *buf)
{
	...
	count += snprintf(tmp + count, PAGE_SIZE,
			"Power Mode:0x%02x\n", regvalue);
