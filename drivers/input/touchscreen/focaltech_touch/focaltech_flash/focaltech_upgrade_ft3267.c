		...
		u8 *vendor_id)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		if ((reg_val[0] == chip_types.bootloader_idh)
			...
			FTS_DEBUG("ID1 = 0x%x, ID2 = 0x%x!!",
					reg_val[0], reg_val[1]);
			break;
		}
		/*else {*/
		FTS_ERROR("[UPGRADE]: read bootloader id fail!!");
		FTS_DEBUG("ID1 = 0x%x, ID2 = 0x%x!!", reg_val[0], reg_val[1]);
		continue;
		/*}*/
	}
	...
	msleep(20); /* 10 must wait, otherwise read vendor id wrong */
...
/*static bool fts_check_app_bin_valid(u8 *buf)
{
	...
	/* 2. len */
	/*len =
		((u16)buf[APP_VERIF_ADDR + APP_LEN] << 8) +
			...
			u8 *pbt_buf, u32 dw_length)
{
	...
	/* if the first byte of app is not 0x02, the app is invaild,
	*  can not upgrade
	*/
	if (pbt_buf[0] != 0x02) {
		FTS_ERROR("[UPGRADE]: app first byte is not 0x02.");
		FTS_ERROR("[UPGRADE]: can not upgrade!!");
		return -EINVAL;/*-1;*/
	}
	...
	if (i >= FTS_UPGRADE_LOOP) {
		FTS_ERROR("[UPGRADE]: get bootload id error:");
		FTS_ERROR("ID1 = 0x%x,ID2 = 0x%x!!", reg_val[0], reg_val[1]);
		return -EIO;
	}
	...
	for (j = 0; j < packet_number; j++) {
		...
		for (i = 0; i < 30; i++) {
			...
			reg_val[1] = 0x00;
			fts_i2c_read(client, auc_i2c_write_buf, 4, reg_val, 2);
			if (0xb0 == (reg_val[0] & 0xf0) && (0x03 +
			(j % 0x0ffd)) == (((reg_val[0] & 0x0f) << 8) |
							reg_val[1])) {
				/*FTS_INFO("[UPGRADE]:
				* write a block data finished!!");
				*/
				break;
			}
			/*msleep(1);*/
