...
void fts_get_chip_types(void)
{
	...
	/* only one array */
	if (sizeof(ctype) != sizeof(struct ft_chip_t))
...
/*int fts_GetFirmwareSize(char *firmware_name)
{
	...
	snprintf(filepath, sizeof(filepath), "%s%s",
		FTXXXX_INI_FILEPATH_CONFIG, firmware_name);
	if (NULL == pfile)
...
enum FW_STATUS fts_ctpm_get_pram_or_rom_id(struct i2c_client *client)
{
	...
	else if (reg_val[0] == chip_types.pramboot_idh && reg_val[1] ==
			chip_types.pramboot_idl)
		inRomBoot = FTS_RUN_IN_PRAM;
	else if (reg_val[0] == chip_types.rom_idh && reg_val[1] ==
			chip_types.rom_idl)
		inRomBoot = FTS_RUN_IN_ROM;
	else if (reg_val[0] == chip_types.bootloader_idh && reg_val[1] ==
			chip_types.bootloader_idl)
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	/* else {*/
		...
		do {
			...
			if (i_ret == 0) {
				/* upgrade success */
				fts_i2c_read_reg(client,
					FTS_REG_FW_VER, &uc_tp_fm_ver);
				/*Upgrade to new version*/
				FTS_DEBUG("[UPGRADE]Upgrade to new ver:0x%x",
						uc_tp_fm_ver);
