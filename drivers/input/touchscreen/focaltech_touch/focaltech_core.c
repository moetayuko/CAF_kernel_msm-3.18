...
#define INTERVAL_READ_REG                   20  /* unit:ms */
#define TIMEOUT_READ_REG                    300 /* unit:ms */
...
static void fts_show_touch_buffer(u8 *buf, int point_num)
{
	...
	count += snprintf(g_sz_debug, PAGE_SIZE, "%02X,%02X,%02X",
			buf[0], buf[1], buf[2]);
	for (i = 0; i < len; i++)
		count += snprintf(g_sz_debug+count, PAGE_SIZE,
			",%02X", buf[i+3]);
...
#if FTS_PSENSOR_EN
	if ((fts_sensor_read_data(data) != 0) &&
			(data->suspended == 1))
		return 1;
#endif
...
static void fts_ts_early_suspend(struct early_suspend *handler)
{
	struct fts_ts_data *data = container_of(handler,
				struct fts_ts_data, early_suspend);
			...
			const struct i2c_device_id *id)
{
	...
	data = devm_kzalloc(&client->dev,
				sizeof(struct fts_ts_data), GFP_KERNEL);
	if (!data) {
		FTS_ERROR("[MEMORY]Failed to allocate memory");
		FTS_FUNC_EXIT();
		return -ENOMEM;
