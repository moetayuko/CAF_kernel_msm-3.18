...
struct fts_esdcheck_st {
	/* 1- esd check active, need check esd 0- no esd check */
	u8	active;
	u8      suspend;
	/* apk or adb is accessing I2C */
	u8      proc_debug;
	/* 1- Interrupt trigger */
	u8      intr;
	u8      unused;
	/* Flow Work Cnt(reg0x91) keep a same value */
	/* for x times. >=5 times is ESD, need reset */
	u8      flow_work_hold_cnt;
	/* Save Flow Work Cnt(reg0x91) value */
...
static bool get_chip_id(void)
{
	...
	if (i >= 3) {
		FTS_ERROR("[ESD]:Read_Chip_id failed, need execute TP reset");
		return 1;
	}
...
static bool get_flow_cnt(void)
{
	...
	if (fts_esdcheck_data.flow_work_hold_cnt >= 5) {
		FTS_DEBUG("[ESD]:Flow_Work Cnt(reg0x91) need execute TP reset");
		return 1;
	}
...
static int esdcheck_algorithm(void)
{
	...
	if (fts_esdcheck_data.suspend == 1) {
		FTS_INFO("[ESD]: In suspend, not check esd, return immedi");
	...
	*/
	if (fts_esdcheck_data.proc_debug == 1) {
		FTS_INFO("[ESD]:In apk or adb command mode, not check esd");
		return 0;
	}

	/* 4. In factory mode, can't check esd, return immediately */
	reg_addr = FTS_REG_WORKMODE;
	err = fts_i2c_read(fts_i2c_client, &reg_addr, 1, &reg_value, 1);
	if (err < 0) {
		fts_esdcheck_data.i2c_nack_cnt++;
	} else if ((reg_value & 0x70) ==  FTS_REG_WORKMODE_FACTORY_VALUE) {
		FTS_INFO("[ESD]:In factory mode, not check esd");
		return 0;
	}
...
*  then need set proc_debug, then will not check ESD.
*  Input:
*  Output:
*  Return:
*****************************************************************************/
