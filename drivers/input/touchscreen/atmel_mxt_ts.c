...
/* Default values */
#define MXT_INVALID_FINGER_ID	(-1)
...
retry_read:
	...
	if (ret != ARRAY_SIZE(xfer)) {
		if (!retry) {
			dev_dbg(&client->dev, "%s: i2c retry reg:%d len:%d\n",
				__func__, reg, len);
...
static void mxt_input_report(struct mxt_data *data)
{
	...
	int finger_num = 0;
	int current_id = MXT_INVALID_FINGER_ID;
	int id;
	...
	for (id = 0; id < data->num_touchids; ++id) {
		...
		if (current_id == MXT_INVALID_FINGER_ID)
			current_id = id;
		else if (id == data->current_finger_id)
			current_id = data->current_finger_id;

		input_report_abs(input_dev, ABS_MT_TOUCH_MAJOR,
				 finger[id].active ? finger[id].area : 0);
	...
	if (finger_num) {
		input_report_abs(input_dev, ABS_X, finger[current_id].x);
		input_report_abs(input_dev, ABS_Y, finger[current_id].y);
		input_report_abs(input_dev,
				 ABS_PRESSURE, finger[current_id].pressure);
		data->current_finger_id = current_id;
	} else {
		data->current_finger_id = MXT_INVALID_FINGER_ID;
	}
...
static void mxt_proc_t100_message(struct mxt_data *data, u8 *message)
{
	...
	finger[id].active = active;
	finger[id].x = x;
	finger[id].y = y;
	finger[id].area = major;
