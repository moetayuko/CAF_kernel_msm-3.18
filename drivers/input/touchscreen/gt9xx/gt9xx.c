...
#if GTP_DEBUG_ON
static const int key_codes[] = {KEY_BACK, KEY_HOMEPAGE, KEY_MENU, KEY_SEARCH};
...
static void gtp_pen_init(struct goodix_ts_data *ts)
{
	...
	input_set_abs_params(ts->input_dev,
				ABS_MT_POSITION_X, ts->pdata->x_min,
				ts->pdata->x_max, 0, 0);
	input_set_abs_params(ts->input_dev,
				ABS_MT_POSITION_Y, ts->pdata->y_min,
				ts->pdata->y_max, 0, 0);
...
static void gtp_int_sync(struct goodix_ts_data *ts, s32 ms)
{
	...
	if (ts->ts_pinctrl) {
		if (pinctrl_select_state(ts->ts_pinctrl,
...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
	...
	/*input_set_abs_params(ts->input_dev,
				 ABS_MT_POSITION_X, 0, ts->abs_x_max,
				     0, 0);
	input_set_abs_params(ts->input_dev,
			 ABS_MT_POSITION_Y, 0, ts->abs_y_max,
			     0, 0);*/
	input_set_abs_params(ts->input_dev,
			ABS_MT_POSITION_X, ts->pdata->x_min,
			ts->pdata->x_max, 0, 0);
...
static int goodix_ts_pinctrl_init(struct goodix_ts_data *goodix_data)
{
	...
	if (IS_ERR_OR_NULL(goodix_data->ts_pinctrl)) {
		...
		dev_dbg(&goodix_data->client->dev,
			"Target does not use pinctrl %d\n", retval);
		goto err_pinctrl_get;
	}

	goodix_data->pinctrl_state_active
		= pinctrl_lookup_state(goodix_data->ts_pinctrl,
				PINCTRL_STATE_ACTIVE);
	...
	goodix_data->pinctrl_state_release
		= pinctrl_lookup_state(goodix_data->ts_pinctrl,
