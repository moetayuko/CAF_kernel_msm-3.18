...
static void time_interval_check(struct pat9125_linux_data_t *ots)
{
...
/* "cat /proc/kmsg" to see kernel message */
	...
	if (OutOtsState == OTS_ROT_UP) {
		/* Right Key */
		input_event(
			pat9125data.pat9125_input_dev, EV_KEY, KEY_RIGHT, 1);
		input_sync(pat9125data.pat9125_input_dev);
		input_event(
			pat9125data.pat9125_input_dev, EV_KEY, KEY_RIGHT, 0);
		input_sync(pat9125data.pat9125_input_dev);
		 pr_debug("Right\n");
	}	else if (OutOtsState == OTS_ROT_DOWN)	{
		/* Left Key */
		input_event(
			pat9125data.pat9125_input_dev, EV_KEY, KEY_LEFT, 1);
		input_sync(pat9125data.pat9125_input_dev);
		input_event(
			pat9125data.pat9125_input_dev, EV_KEY, KEY_LEFT, 0);
		input_sync(pat9125data.pat9125_input_dev);
		 pr_debug("Left\n");
	}

	if (OutBtnState == OTS_BTN_RELEASE)	{
		input_event(pat9125data.pat9125_input_dev,
			EV_KEY, KEY_ENTER, 1);
		input_sync(pat9125data.pat9125_input_dev);
		input_event(pat9125data.pat9125_input_dev,
			EV_KEY, KEY_ENTER, 0);
		input_sync(pat9125data.pat9125_input_dev);
		 pr_debug("Enter Released\n");
	}
