 ...
 * Insmod parameters
 */
I2C_CLIENT_INSMOD_1(smb329_i2c);
...
int set_charger_ctrl(uint32_t ctl)
{
	atomic_set(&smb329.chg_state, ctl);
...
{
	uint32_t chg_state;
	struct smb329_data *smb329p =
	    container_of(work, struct smb329_data, work);

	if (!smb329p)
		return;

	chg_state = atomic_read(&smb329p->chg_state);
...
static void __exit smb329_exit(void)
{
