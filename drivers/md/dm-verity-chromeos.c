...
static int chromeos_invalidate_kernel_bio(struct block_device *root_bdev)
{
	 ...
	 */
	int flags = REQ_SYNC | REQ_SOFTBARRIER | REQ_NOIDLE;
	...
	if (chromeos_invalidate_kernel_submit(bio, bdev, flags, REQ_OP_READ,
					      page)) {
		ret = -1;
	...
	if (chromeos_invalidate_kernel_submit(bio, bdev, flags, REQ_OP_WRITE,
					      page)) {
		ret = -1;
