...
struct rmnet_private
{
	...
	ktime_t last_packet;
	short active_count; /* Number of times left to check */
	short session_count; /* Number of polls seems so far */
...
static void do_check_active(struct work_struct *work)
{
	...
	if (in_suspend) {
		 ...
		 */
		int tmp;
		tmp = p->timeout_us * 2 - (p->timeout_us / (p->active_count + 1));
		tmp /= 1000;
		p->awake_time_ms += tmp;

		p->active_count = p->session_count = 0;
		return;
	}
	...
	p->session_count++;
	if (--p->active_count == 0) {
		p->awake_time_ms += p->session_count * POLL_DELAY / 1000;
		p->session_count = 0;
	} else {
...
static int rmnet_cause_wakeup(struct rmnet_private *p) {
	...
	if (in_suspend)
		...
		p->active_count = p->timeout_us / POLL_DELAY;
