...
static int thirdparty_bl_set(struct mdss_dsi_ctrl_pdata *ctrl, int level)
{
	...
	static int pulse=0;
	static int pre_pulse = 0;
	static int menu_flag = 0;
	//static struct msm_panel_common_pdata *sgm3131_backlight_pdata;
	...
	if (boot_flag == 0){
		...
		pr_err("@@ backlight gpio is %d",bl_ctl_num);
		ret = gpio_request(bl_ctl_num,"gpio3");
		if (ret)
			printk("@@ gpio3 request error!");
		ret = gpio_direction_output(bl_ctl_num, 0);
		if (ret)
			printk("@@ gpio3 set direction output error!");
	}
	if (level > max)
	    level = max;
	if (level < min)
...
#if 0 // Thirdparty backlight is required using in XP6
			if (gpio_is_valid(ctrl_pdata->bklt_en_gpio))
