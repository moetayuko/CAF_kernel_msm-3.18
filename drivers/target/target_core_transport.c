 ...
 * @cmd: Drop a reference obtained by target_get_sess_cmd().
 *
 * Returns 1 if and only if this target_put_sess_cmd() call caused the
 * refcount to drop to zero. Returns zero otherwise.
 */
int target_put_sess_cmd(struct se_cmd *cmd)
{
	long ref = atomic_dec_return(&cmd->tgt_ref);

	WARN_ON_ONCE(ref < 0);
	if (ref == 0)
		complete(&cmd->complete);

	return __target_put_sess_cmd(cmd);
}
