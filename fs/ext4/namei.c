	...
	while ((char *) de < base + size)
	{
		if (de->inode)
		{
			if (show_names)
			{
				...
				int len;
				char *name;
				struct ext4_cstr fname_crypto_str
					= {.name = NULL, .len = 0};
				...
				if (ctx == NULL) {
					...
					res = ext4_fname_disk_to_usr(ctx,
						&fname_crypto_str, &tmp_str);
					if (res < 0) {
						printk(KERN_WARNING "Error "
							"converting filename "
							"from disk to usr"
					...
					while (len--)
						printk("%c", *name++);
					res = ext4_fname_disk_to_htree(ctx,
						&fname_crypto_str, &tmp_str);
					if (res < 0) {
						printk(KERN_WARNING "Error "
							"converting filename "
	 ...
	 struct dx_hash_info *hinfo, struct dx_frame *frame_in)
{
...
#ifdef CONFIG_EXT4_FS_ENCRYPTION
	struct ext4_fname_crypto_ctx *ctx = NULL;
	struct ext4_cstr fname_crypto_str = {.name = NULL, .len = 0}, tmp_str;
	int res;
#endif
	...
	if (d_name) {
		...
		if (ctx == NULL) {
			...
			res = ext4_fname_crypto_alloc_buffer(ctx,
				&fname_crypto_str.name,
				&fname_crypto_str.len, d_name->len);
			if (res < 0) {
				ret_err = ERR_PTR(res);
				goto fail;
			}
			tmp_str.name = (unsigned char *)d_name->name;
			tmp_str.len = d_name->len;
			res = ext4_fname_usr_to_htree(ctx, &fname_crypto_str,
				&tmp_str);
			if (res < 0) {
				ret_err = ERR_PTR(res);
				goto fail;
	...
	if (root->info.unused_flags & 1) {
		ext4_warning(dir->i_sb, "Unimplemented inode hash flags: "
			     "%#06x", root->info.unused_flags);
		goto fail;
	}

	if ((indirect = root->info.indirect_levels) > 1) {
		ext4_warning(dir->i_sb, "Unimplemented inode hash depth: "
			     "%#06x", root->info.indirect_levels);
		goto fail;
	}
	...
	while (1) {
		...
		at = p - 1;
		dxtrace(printk(" %x->%u\n", (at == entries)
			? 0 : dx_get_hash(at), dx_get_block(at)));
				  ...
				  __u32 start_hash, __u32 start_minor_hash)
{
	...
	for (; de < top; de = ext4_next_entry(de, dir->i_sb->s_blocksize)) {
		...
		if (ctx == NULL) {
			...
			tmp_str.name = de->name;
			tmp_str.len = de->name_len;
			err = ext4_fname_disk_to_htree(ctx,
					&fname_crypto_str, &tmp_str);
			if (err < 0) {
				ext4_put_fname_crypto_ctx(&ctx);
				ext4_fname_crypto_free_buffer(
		...
		if (ctx == NULL) {
			...
			tmp_str.name = de->name;
			tmp_str.len = de->name_len;
			err = ext4_fname_disk_to_usr(ctx, &fname_crypto_str,
				&tmp_str);
			if (err < 0) {
				ext4_put_fname_crypto_ctx(&ctx);
				ext4_fname_crypto_free_buffer(
...
{
	int res;
	struct ext4_cstr tmp_str = {.name = de->name, .len = de->name_len};
	...
	/* Directory is encrypted */
	res = ext4_fname_disk_to_usr(ctx, fname_crypto_str, &tmp_str);
	if (res < 0)
		return res;
	if (len != res)
		return 0;
...
#ifdef CONFIG_EXT4_FS_ENCRYPTION
static int search_dir_crypt(struct buffer_head *bh,
			    char *search_buf,
			    int buf_size,
			    struct inode *dir,
			    const struct qstr *d_name,
			    ...
			    struct ext4_dir_entry_2 **res_dir)
{
	...
	struct ext4_fname_crypto_ctx *ctx = NULL;
	struct ext4_cstr fname_crypto_str = {.name = NULL, .len = 0};
	int res;
...
static int search_dir_noncrypt(struct buffer_head *bh,
			       char *search_buf,
			       int buf_size,
			       struct inode *dir,
			       const struct qstr *d_name,
...
{
#ifdef CONFIG_EXT4_FS_ENCRYPTION
	return search_dir_crypt(bh, search_buf, buf_size, dir, d_name, offset,
				res_dir);
#else
	return search_dir_noncrypt(bh, search_buf, buf_size, dir, d_name,
				   offset, res_dir);
#endif
}
			...
			struct dx_hash_info *hinfo)
{
	...
	dxtrace(dx_show_leaf(hinfo, (struct ext4_dir_entry_2 *) data1,
			blocksize, 1));
	dxtrace(dx_show_leaf(hinfo, (struct ext4_dir_entry_2 *) data2,
			blocksize, 1));
#endif
			    ...
			    struct ext4_dir_entry_2 **dest_de)
{
	...
	if (ctx != NULL) {
		...
		reclen = EXT4_DIR_REC_LEN(
		    ext4_fname_crypto_namelen_on_disk(ctx, namelen));
		if (reclen < 0) {
			ext4_put_fname_crypto_ctx(&ctx);
			return reclen;
		}
...
int ext4_find_dest_de_noncrypt(struct inode *dir, struct inode *inode,
			       struct buffer_head *bh,
			       void *buf, int buf_size,
			       const char *name, int namelen,
			       struct ext4_dir_entry_2 **dest_de)
...
{
#ifdef CONFIG_EXT4_FS_ENCRYPTION
	return ext4_find_dest_de_crypt(dir, inode, bh, buf, buf_size, name,
				       namelen, dest_de);
#else
	return ext4_find_dest_de_noncrypt(dir, inode, bh, buf, buf_size, name,
					  namelen, dest_de);
#endif
}
...
#endif
{
	...
	if (ctx != NULL) {
		...
		res = ext4_fname_usr_to_disk(ctx, &fname_crypto_str,
				&tmp_str);
		if (res < 0) {
			ext4_put_fname_crypto_ctx(&ctx);
			ext4_fname_crypto_free_buffer(
...
#ifdef CONFIG_EXT4_FS_ENCRYPTION
	err = ext4_insert_dentry(dir, inode, de, blocksize, name, namelen);
	if (err < 0)
		return err;
#else
			    ...
			    struct inode *inode, struct buffer_head *bh)
{
	...
	if (ctx == NULL) {
		...
		tmp_str.name = (unsigned char *)name;
		tmp_str.len = namelen;
		res = ext4_fname_usr_to_htree(ctx,
					      &fname_crypto_str,
					      &tmp_str);
		if (res < 0) {
			ext4_put_fname_crypto_ctx(&ctx);
