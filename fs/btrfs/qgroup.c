...
/*
 * Walk up the tree from the bottom, freeing leaves and any interior nodes
 * which have had all slots visited. If a node (leaf or interior) is freed, the
 * node above it will have it's slot incremented. The root node will never be
 * freed.
 *
 * At the end of this function, we should have a path which has all slots
 * incremented to the next position for a search. If we need to read a new node
 * it will be NULL and the node above it will have the correct slot selected
 * for a later read.
 *
 * If we increment the root nodes slot counter past the number of elements, 1
 * is returned to signal completion of the search.
 */
				...
				struct btrfs_path *path, int root_level)
{
	...
	while (level <= root_level) {
		...
		if (slot >= nr || level == 0) {
			/*
			 * Don't free the root -  we will detect this condition
			 * after our loop and return a positive value for
			 * caller to stop walking the tree.
			 */
			if (level != root_level) {
			...
			/*
			 * We have a valid slot to walk back down from. Stop
			 * here so caller can process these new nodes.
			 */
			break;
		}
			       ...
			       u64 root_gen, int root_level)
{
	...
	/*
	 * Walk down the tree.  Missing extent blocks are filled in as we go.
	 * Metadata is accounted every time we read a new extent block.
	 *
	 * When we reach a leaf, we account for file extent items in it, walk
	 * back up the tree (adjusting slot pointers as we go) and restart the
	 * search process.
	 */
