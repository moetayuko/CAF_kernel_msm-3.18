...
static int audio_dsp_set_agc(struct audio_in *audio)
{
	...
	if (audio->agc_enable) {
		...
		memcpy(&cmd.static_gain, &audio->agc.agc_params[0], sizeof(uint16_t) * 6);
		...
		memcpy(&cmd.aig_attackk, &audio->agc.agc_params[6], sizeof(uint16_t) * 14);
...
static int audio_dsp_set_ns(struct audio_in *audio)
{
	...
	if (audio->ns_enable) {
		...
		memcpy(&cmd.dens_gamma_n, &audio->ns.ns_params, sizeof(audio->ns.ns_params));
	} else {
		cmd.ec_mode_new =
...
static int audio_dsp_set_tx_iir(struct audio_in *audio)
{
	...
	if (audio->iir_enable) {
		...
		memcpy(&cmd.iir_params, &audio->iir.iir_params, sizeof(audio->iir.iir_params));
	} else {
...
static long audpre_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case AUDIO_SET_AGC: {
		if (copy_from_user(&audio->agc, (void *) arg, sizeof(audio->agc)))
			goto out_fault;
#if DEBUG
		pr_info("set agc\n");
