...
#if defined(DEBUG)
#define DBG(fmt, x...) \
	do { pr_info("%s: " fmt, __func__, ##x); } while (0)
#else
...
static int hw3d_open(struct inode *inode, struct file *file)
{
	...
	if (info->client_file) {
		DBG("have client_file, need revoke\n");
...
static void hw3d_vma_close(struct vm_area_struct *vma)
{
	...
	DBG("current %u ppid %u file %p count %ld\n",
...
static void hw3d_early_suspend(struct early_suspend *h)
{
	...
	info->suspending = 1;
	if (info->client_file) {
		DBG("Requesting revoke for suspend\n");
		locked_hw3d_revoke(info);
	}
	spin_unlock_irqrestore(&info->lock, flags);
}
