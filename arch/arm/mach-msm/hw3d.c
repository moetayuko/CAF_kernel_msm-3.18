		      ...
		      unsigned long *len, struct file **filp)
{
	...
	if (unlikely(region >= HW3D_NUM_REGIONS)) {
		VDBG("invalid region %d requested\n", region);
		return -EINVAL;
	} else if (unlikely(offs >= info->regions[region].size)) {
		VDBG("offset %08x outside of the requested region %d\n", offs,
		     region);
		return -EINVAL;
	}
	...
	if (unlikely(!data->vmas[region])) {
		...
		VDBG("requested hw3d region is not mapped\n");
		ret = -ENOENT;
		goto err;
	}
...
static int hw3d_open(struct inode *inode, struct file *file)
{
	...
	if (info->client_file) {
		pr_debug("have client_file, need revoke\n");
...
static void hw3d_vma_close(struct vm_area_struct *vma)
{
	...
	pr_debug("current %u ppid %u file %p count %ld\n",
...
static void hw3d_early_suspend(struct early_suspend *h)
{
	...
	info->suspending = 1;
	if (info->client_file) {
		pr_debug("Requesting revoke for suspend\n");
		locked_hw3d_revoke(info);
	}
	spin_unlock_irqrestore(&info->lock, flags);
}
