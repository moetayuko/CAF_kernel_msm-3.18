...
static void dsp_assert_nmi(void)
{
	void __iomem *ptr;

	ptr = ioremap(0xA8700000, PAGE_SIZE);
	if (!ptr) {
		pr_err("dsp: cannot map registers\n");
		return;
	}
			 ...
			 size_t count, loff_t *pos)
{
	...
	if (!strcmp(cmd, "nmi"))
		dsp_assert_nmi();
	else if (!strcmp(cmd, "wait-for-crash")) {
		while (!dsp_has_crashed) {
			dsp_wait_count++;
			wait_event(dsp_wait, dsp_has_crashed);
			dsp_wait_count--;
		}
	} else if (!strcmp(cmd, "boom")) {
		dsp_has_crashed = 1;
		wake_up(&dsp_wait);
	} else if (!strcmp(cmd, "boom2")) {
...
#define DSP_RAM_BASE 0x2E000000
#define DSP_RAM_SIZE 0x02000000
