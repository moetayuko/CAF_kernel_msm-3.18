...
#ifdef CONFIG_SII6031_MHL_SWITCH
...
#ifdef CONFIG_SII6031_MHL_SWITCH
/**
 * msm_otg_register_mhl_discovery: mhl driver calls this function to register
 * it's discovey function.
 *
 * mhl driver integration makes use of the struct usb_ext_notification to
 * store function and it's context. This method is a bit reverse of what
 * intended use of usb_ext_notification was. Discovery called via
 * function pointer allows dynamic linking of "any" mhl driver.
 *
 * @ctxt: mhl driver's context pointer
 * @mhl_discover_device: pointer to the function that performs mhl discovery.
 *          Returns 1 if MHL is discovered.
 * @usb_ext_allocated: mhl driver allocates and sends back the pointer for
 * usb_ext_notification.
 */
static struct usb_ext_notification *mhl_ext;

int otg_register_mhl_discovery(
	void *ctxt,
	int (*mhl_discover_device)(void *, int on,
		void (*mhl_cb)(void *ctxt, int online), void *))
{
	...
	dev_info(phy->dev, "SIMG: msm_otg_register\n");
	if (mhl_ext) {
		pr_debug("%s: already registered\n", __func__);
		return -EEXIST;
	}

	if (!mhl_discover_device || !ctxt) {
		pr_debug("%s: invalid parameter(s)\n", __func__);
		return -EINVAL;
	}
	mhl_ext = kzalloc(sizeof(struct usb_ext_notification), GFP_KERNEL);
	if (!mhl_ext)
		return -ENOMEM;

	mhl_ext->notify = mhl_discover_device;
	mhl_ext->ctxt = ctxt;
	clear_bit(MHL,&motg->inputs);

	/*
	 * start in MHL mode first - what if MHL cable is already connected
	 *	and no interrupt will be asserted.
	 */
	sii_switch_to_mhl(true);

	return 0;
}
EXPORT_SYMBOL(otg_register_mhl_discovery);

int otg_unregister_mhl_discovery(void)
{
	printk("SIMG: msm_otg_unregister\n");
	kfree(mhl_ext);
	mhl_ext = 0;
	return 0;
}
EXPORT_SYMBOL(otg_unregister_mhl_discovery);
...
void otg_mhl_notify(void *ctxt, int on)
{
	...
	if (on) {
		dev_info(phy->dev, "SIMG: found MHL, set bit.\n");
		set_bit(MHL,&motg->inputs);
	}
	else {
		/* Ask Silicon Image 6031 driver to steer switch to USB */
		dev_info(phy->dev,
...
EXPORT_SYMBOL(otg_mhl_notify);
#else
inline void msm_otg_mhl_notify(int on)
{
	return;
}
EXPORT_SYMBOL(msm_otg_mhl_notify);
#endif
...
static void msm_otg_sm_work(struct work_struct *w)
{
	...
	case OTG_STATE_B_IDLE:
		if (test_bit(MHL, &motg->inputs)) {
			...
			pr_debug("!id || id_A\n");
#ifdef CONFIG_SII6031_MHL_SWITCH
	dev_info(phy->dev, "Switch Stark to MHL then perform MHL discovery\n");
				if(mhl_ext){
					/* ask Silicon Image 6031 driver to steer
					 * switch to MHL
					 */
					sii_switch_to_mhl(true);
					/* ask Silicon Image mhl driver to discover mhl
					 * (returns >0 if MHL)
					*/
					ret = mhl_ext->notify(mhl_ext->ctxt,
						MHL, otg_mhl_notify, motg);
					if(ret>0){
						work =1;
						break;
					}
					else
						pr_debug("mhl Discovery failed!\n");
				}
#endif
			if (msm_chg_mhl_detect(motg)) {
				work = 1;
...
#ifdef CONFIG_SII6031_MHL_SWITCH
	dev_info(phy->dev, "SIMG: Switch Stark to USB - NOT PERFORMED HERE\n");
			clear_bit(MHL, &motg->inputs);
#endif
			work = 1;
		}
